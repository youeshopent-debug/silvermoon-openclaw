...
function loadDotEnv() {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const idx = s.indexOf('=');
      if (idx <= 0) continue;
      const key = s.slice(0, idx).trim().replace(/^\uFEFF/, '');
...
function loadUserDataEnvIfNeeded() {
  ...
  try {
    ...
    for (const line0 of lines) {
      ...
      const idx = t.indexOf('=');
      if (idx <= 0) continue;
      const k = t.slice(0, idx).trim().replace(/^\uFEFF/, '');
      let v = t.slice(idx + 1).trim();
      if (!k) continue;
      if (v.length >= 2) {
        if ((v.startsWith('"') && v.endsWith('"')) || (v.startsWith("'") && v.endsWith("'"))) v = v.slice(1, -1);
      }
      if (process.env[k] == null || String(process.env[k]).length === 0) {
        process.env[k] = v;
        changed = true;
      }
    }
...
function buildDiscordSendOptions(rawText, channelId) {
  ...
  if (!wantsAttach) {
    const content = sanitizeDiscordReply(s, { verbose, channelId: cid });
    return { content };
  }
  ...
  if (!delivery) {
    ...
    const content = sanitizeDiscordReply(enforceDeliveryGuard(s, { verbose }), { verbose, channelId: cid });
    return { content };
  }
  ...
  if (typeof delivery.size === 'number' && delivery.size > DISCORD_MAX_UPLOAD_BYTES) {
    ...
    const content = sanitizeDiscordReply(msg, { verbose, channelId: cid });
    return { content };
  }
...
async function selfCheckConnectivity() {
  ...
  try {
    ...
    STATE.selfCheck.ollama = { ok, detail: ok ? `${dt}ms` : `status:${resp.status}`, at: nowIso };
  } catch (e) {
...
async function refreshOllamaModels() {
  try {
    ...
    const dt = Date.now() - t0;
    if (resp.status !== 200 || !resp.json || !Array.isArray(resp.json.models)) return;
    STATE.ollama.models = resp.json.models.map((m) => m.name).filter(Boolean);
...
function sanitizeDiscordReply(text, opts) {
  ...
  const allowMore =
    ...
    /(银月情报局|斗湖早报|早晨简报|23:55\s*汇总|夜报|每日汇总)/.test(out);
...
function buildAgentInstruction(targetAgent, modeInstruction, ctx) {
  ...
  const behavior = [
    ...
    '【追问】允许提问，但每轮最多 1 个、且必须是推进话题的具体问题；禁止“您想聊什么/我还能帮什么”',
    '【反甩锅】禁止输出“请提供背景/请在终端运行/请自行查找/请查看数据库”等把任务转嫁给主人的话术；必须改为：先交付一个可用默认方案 + 仅追问 1 个关键点',
    '【表情】每条回复 Emoji ≤ 2',
...
function summarizeToolResult(name, r) {
  const n = String(name || '').trim();
  if (!r || typeof r !== 'object') return `tool=${n}：无结果`;
  if (r.ok === false) return `tool=${n}：不可用（${String(r.reason || r.error || 'unknown')}）`;
  if (n === 'tailErrors') {
    const t = String(r.text || '').trim();
    const head = t ? t.split('\n').slice(0, 20).join('\n') : '';
    return `tool=${n}：最近错误摘要\n${head}`.trim();
  }
  if (n === 'readFileSafe') {
    const t = String(r.text || '').trim();
    const head = t ? t.split('\n').slice(0, 30).join('\n') : '';
    return `tool=${n}：文件=${String(r.path || '')}\n${head}`.trim();
  }
  if (n === 'webSearch') {
    ...
    return `tool=${n}：query=${String(r.query || '')}\n${lines.join('\n')}`.trim();
  }
  if (n === 'proposeExec') {
...
async function askSilvermoonAutonomyD({ userText, extraInstruction, channelId, requestedBy }) {
  ...
  const evidence = results.map((x) => summarizeToolResult(x.name, x.result)).join('\n\n');
  const finalPrompt = [
...
async function replyAndRemember(msg, text) {
  const payload0 = buildDiscordSendOptions(text, msg.channelId);
  ...
  const compact = isModelDownNoticeText(raw)
    ...
    : raw;
  const payload = compact !== raw ? buildDiscordSendOptions(compact, msg.channelId) : payload0;
  ...
  if (shouldSuppressDuplicateAssistantReply(msg.channelId, payload?.content || '', ttlMs, uHash)) {
    if (!isPing && !shouldSuppressNotice(msg.channelId, 'reply_suppressed_ack', 15_000)) {
      const ack = buildDiscordSendOptions('🕓 已收到（重复请求已合并）。', msg.channelId);

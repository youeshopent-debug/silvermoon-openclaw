    ...
    for (const re of tool.triggers) {
      if (re.test(cleaned)) {
        ...
        try {
          ...
          try { selfHeal.handleError(err, { tool: tool.name, text: cleaned }); } catch {}
          const errorMsg = {
...
function registerChromeTools(chrome) {
  register({
    name: 'chrome_launch',
    description: '启动 Chrome 浏览器（Profile 3, 远程调试端口 9222）',
    triggers: [
      /^(拉起|启动|打开)\s*(chrome|浏览器)/i,
      /^(launch|start|open)\s*(chrome|browser)/i,
    ],
    handler: async () => {
      const cp = require('child_process');
      const path = require('path');
      const scriptPath = path.join(__dirname, '..', 'scripts', 'launch-chrome.cjs');
      return new Promise((resolve) => {
        cp.execFile(process.execPath, [scriptPath], { timeout: 20000 }, (err, stdout, stderr) => {
          if (err) {
  ...
  register({
    name: 'chrome_close',
    description: '关闭 Chrome 浏览器（杀掉所有 Chrome 进程）',
    triggers: [
      /^(关闭|停止)\s*(chrome|浏览器)/i,
      /^(close|stop|kill)\s*(chrome|browser)/i,
    ],
    handler: async () => {
      const cp = require('child_process');
      return new Promise((resolve) => {
        cp.exec('taskkill /f /im chrome.exe', { timeout: 10000 }, (err) => {
          if (!err) {
            resolve('✅ 已关闭 Chrome 浏览器');
          } else {
            // 没有 Chrome 进程也算成功
            resolve('✅ Chrome 进程已终止（或未运行）');
          }
        });
      });
    },
  ...
  register({
    ...
    description: '断开 Chrome CDP 连接（不关闭浏览器）',
    triggers: [
      /^(断开)\s*(chrome|浏览器)/i,
      /^disconnect\s*(chrome|browser)/i,
    ],
    handler: async () => {
      await chrome.disconnect();
      return '✅ 已断开 Chrome CDP 连接';
    },
  });

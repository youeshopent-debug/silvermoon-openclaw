...
function applyTaskContractToReply(params) {
  ...
  const addChecklist = String(process.env.OPENCLAW_TASK_CONTRACT_CHECKLIST || '').trim() === '1';
  if (addChecklist && kind === 'advisory' && out.length >= 60 && !/[？?]/.test(out)) out = ensureAcceptanceChecklist(out);

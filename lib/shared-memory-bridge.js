...
class SharedMemoryBridge {
  ...
  write(opts) {
    ...
    try {
      ...
      if (out.changes > 0) this._scheduleRefresh();
  ...
  markActive(agentId) {
    ...
    try { this.stmtUpdateActive.run(agentId); } catch (e) { console.warn('[shared-memory-bridge] markActive error:', e?.message || e); }
  }
  ...
  _refreshSummary() {
    try {
      ...
      const tmp = SUMMARY_PATH + '.tmp.' + Date.now();
      fs.writeFileSync(tmp, lines.join('\n'), 'utf8');
      fs.renameSync(tmp, SUMMARY_PATH);
    } catch (e) { console.warn('[shared-memory-bridge] _refreshSummary write error:', e?.message || e); }
  }
  ...
  getSummary() {
    if (fs.existsSync(SUMMARY_PATH)) {
      ...
      if (raw.length < 50) {
        console.warn('[shared-memory-bridge] getSummary: SHARED_MEMORY.md too short (%d bytes), ignoring', raw.length);
        return null;
      }
      return raw;
    }
  ...
  close() {
    try { if (this.db) this.db.close(); } catch (e) { console.warn('[shared-memory-bridge] close error:', e?.message || e); }
    this.db = null;
    this.ready = false;
  }
}

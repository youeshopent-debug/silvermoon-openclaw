 ...
 */
function switchAgent(channelId, agentName) {
  // 支持模糊匹配
  let matched = registry.get(agentName);

  if (!matched) {
    // 尝试部分匹配
    for (const [name, config] of registry.entries()) {
      if (
        name.includes(agentName) ||
        (config.nameEn && config.nameEn.toLowerCase().includes(agentName.toLowerCase()))
      ) {
        matched = config;
        break;
      }
...
function detectAgentSwitch(text) {
  ...
  // 中文：切换到银月 / 让李长寿来 / 召唤美杜莎
  const zhMatch = text.match(/(?:切换|换|召唤|请|让|叫)\s*(?:到|为|成)?\s*([^\s,，。！!?？]{1,6})\s*(?:来|出来|上线)?/);
  if (zhMatch) {
    return { shouldSwitch: true, targetAgent: zhMatch[1].trim() };
  }

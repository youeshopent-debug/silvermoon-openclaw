 ...
 * 格式化天气查询结果
 * @param {object} deps
 * @param {Function} deps.fetchWeather - 天气获取函数，返回 { city, temp, condition, humidity }
 */
async function handleWeatherQuery({ fetchWeather, lang }) {
  const t = TEMPLATES[lang] || TEMPLATES.zh;
  try {
    if (typeof fetchWeather !== 'function') return t.errorWeather;
    const w = await fetchWeather();
    if (!w) return t.errorWeather;
    const lines = [
      t.weatherTitle,
      '',
      `🔹 ${t.weatherCity}：${w.city || '-'}`,

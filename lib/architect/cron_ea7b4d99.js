 ...
 * @param {Function} [deps.buildMorningBrief]   - 构建早报
 * @param {Function} [deps.buildNightlyReport]  - 构建夜报
 * @param {Function} [deps.consolidateMemory]   - 整理记忆
 * @param {Function} [deps.fetchJobIntel]       - 情报收集
 * @param {Function} [deps.sendToChannel]       - 发送到 Discord 频道
 * @param {string}   [deps.briefChannelId]      - 早报/夜报目标频道
 */
function registerPresets(deps = {}) {
  const channelId = deps.briefChannelId;

  // 早报 — 每天 08:00
  if (deps.buildMorningBrief && deps.sendToChannel && channelId) {
    register({
      name: 'morning_brief',
      schedule: '08:00',
      handler: async () => {
        console.log('[cron] 执行早报任务...');
        try {
          const brief = await deps.buildMorningBrief();
          if (brief) await deps.sendToChannel(channelId, brief);
        } catch (e) {
          console.error('[cron] 早报任务失败:', e.message);
        }
      },
    });
  }
  ...
  // 夜报 — 每天 23:55
  if (deps.buildNightlyReport && deps.sendToChannel && channelId) {
    register({
      name: 'nightly_report',
      schedule: '23:55',
      handler: async () => {
        console.log('[cron] 执行夜报任务...');
        try {
          const report = await deps.buildNightlyReport();
          if (report) await deps.sendToChannel(channelId, report);
        } catch (e) {
          console.error('[cron] 夜报任务失败:', e.message);
        }
      },
    });
  }

...
async function discoverCDPEndpoint() {
  ...
  if (!info.webSocketDebuggerUrl) {
    throw new Error('Chrome CDP 未就绪。请在 Telegram 说「拉起浏览器」或运行: node scripts/launch-chrome.cjs');
  }
...
async function connect(targetUrl) {
  if (_browser) {
    try { await _browser.pages(); return { ok: true }; }
  ...
  return { ok: true, tabs: pages.length, mode: 'connect' };
}

async function launchOrConnect() {
  if (_browser) {
    try { await _browser.pages(); return { mode: 'connect' }; }
    catch { _browser = null; _page = null; }
  }

  // 仅连接已有 Chrome CDP（不自动启动）
  const wsEndpoint = await discoverCDPEndpoint();
  _browser = await puppeteer.connect({
    browserWSEndpoint: wsEndpoint,
    defaultViewport: null,
  ...
  const target = _browser.target();
  _targetId = target._targetId || null;
  return { mode: 'connect', tabs: pages.length };
}

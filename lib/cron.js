...
const CRON_LOG = path.join(__dirname, '..', 'workspace', 'CRON', 'cron_exec.log');
function ensureLogDir() {
  try { fs.mkdirSync(path.dirname(CRON_LOG), { recursive: true }); } catch {}
}
...
function register(task) {
  ...
  const wrappedHandler = async () => {
    ...
    try {
      ...
      const line = `[${new Date().toISOString()}] OK  ${task.name}  (${ms}ms)`;
      fs.appendFileSync(CRON_LOG, line + '\n', 'utf-8');
      console.log(line);
    } catch (e) {
      const ms = Date.now() - t0;
      const msg = String(e?.stack || e?.message || e || '').slice(0, 300);
      const line = `[${new Date().toISOString()}] FAIL ${task.name}  (${ms}ms) ${msg}`;
      fs.appendFileSync(CRON_LOG, line + '\n', 'utf-8');
      console.error(line);
    }
  };
...
function startAll() {
  ...
  for (const [name, task] of tasks.entries()) {
    ...
    if (intervalMs > 0) {
      if (!task._lastRun) task._lastRun = Date.now();
      const timer = setInterval(async () => {
        if (!task.enabled) return;
        task._lastRun = Date.now();
        try { await task.handler(); }
        catch (e) { console.error(`[cron] 任务 ${name} 执行出错:`, e.message); }
      }, intervalMs);
...
/**
 * 启动后检查并补跑今日已错过的定点任务（HH:MM 格式）
 * 防止因进程重启导致早于启动时间的任务被跳过
 */
function catchUpMissedTasks() {
  const now = new Date();
  const todayStr = now.toISOString().slice(0, 10);
  const tz = 'Asia/Kuala_Lumpur';

  const currentMYT = now.toLocaleTimeString('en-GB', {
    timeZone: tz,
    hour: '2-digit',
    minute: '2-digit',
    hour12: false,
  ...
  // 读取 cron_exec.log 判断今日是否已执行过
  let todayExecuted = new Set();
  try {
    const log = fs.readFileSync(CRON_LOG, 'utf-8');
    for (const line of log.split('\n')) {
      if (line.includes(todayStr)) {
        const m = line.match(/(?:OK|FAIL)\s+(\S+)/);
        if (m) todayExecuted.add(m[1]);
      }
    }
  } catch {}
  ...
  let pending = [];
  let delay = 0;
  const STAGGER = 15000; // 每个任务间隔 15 秒
  ...
  for (const [name, task] of tasks.entries()) {
    ...
    // 解析调度时间
    const dayTimeMatch = task.schedule.match(/^(Sun|Mon|Tue|Wed|Thu|Fri|Sat)\s+(\d{1,2}):(\d{2})$/i);
    let scheduledTime;
    if (dayTimeMatch) {
      const dayAbbr = dayTimeMatch[1].charAt(0).toUpperCase() + dayTimeMatch[1].slice(1).toLowerCase();
      const localDay = now.toLocaleDateString('en-GB', { timeZone: tz, weekday: 'short' });
      if (localDay !== dayAbbr) continue;
      scheduledTime = `${dayTimeMatch[2].padStart(2, '0')}:${dayTimeMatch[3]}`;
    } else {
      const timeMatch = task.schedule.match(/^(\d{1,2}):(\d{2})$/);
      if (!timeMatch) continue;
      scheduledTime = task.schedule.padStart(5, '0');
    }

    if (scheduledTime >= currentMYT) continue;
    if (todayExecuted.has(name)) continue;

    pending.push({ name, scheduledTime, runAt: delay });
    delay += STAGGER;
  }

  if (pending.length === 0) return;

  console.log(`[cron:catchup] 📋 ${pending.length} 个今日错过的任务将在启动后补跑`);
  for (const { name, scheduledTime, runAt } of pending) {
    setTimeout(() => {
      console.log(`[cron:catchup] ⏰ 补跑: ${name} (原计划 ${scheduledTime})`);
      const task = tasks.get(name);
      if (task) task.handler().catch(() => {});
    }, runAt);
  }
}

/**
 * 每分钟执行一次：匹配定点任务 + 守护士（watchdog）
 */
function tick() {
  const now = Date.now();
  ...
  for (const [name, task] of tasks.entries()) {
    ...
    // ── 守护士：检查 interval 任务的定时器是否存活 ──
    const intervalMs = parseIntervalSchedule(task.schedule);
    if (intervalMs > 0) {
      const hasTimer = timers.has(name);
      const stale = task._lastRun > 0 && (now - task._lastRun) > intervalMs * 1.5;

      if (!hasTimer || stale) {
        if (!hasTimer) {
          console.log(`[cron:watchdog] ⚠️ 间隔任务 ${name} 的定时器已丢失，正在重建（上次执行: ${task._lastRun ? new Date(task._lastRun).toISOString() : '从未'})`);
        } else {
          console.log(`[cron:watchdog] ⚠️ 间隔任务 ${name} 已 ${((now - task._lastRun) / 1000 / 60).toFixed(1)} 分钟未执行（预定间隔 ${intervalMs / 1000 / 60} 分钟），正在重建`);
        }
        ...
        const timer = setInterval(async () => {
          ...
          try {
            ...
            console.error(`[cron:watchdog] 任务 ${name} 执行出错:`, e.message);
          }
        }, intervalMs);

        timers.set(name, timer);
        console.log(`[cron:watchdog] ✅ 间隔任务 ${name} 定时器已重建 (每 ${intervalMs / 1000 / 60} 分钟)`);
      }
      continue; // 跳过定点检查
    }

    // ── 定点任务匹配 ──
    if (shouldRunAt(task, new Date(now))) {
      const minuteKey = `${new Date(now).getHours()}:${new Date(now).getMinutes()}`;
...
function parseIntervalSchedule(schedule) {
  const match = String(schedule || '').match(/every\s+(\d+)\s*(d|h|m|s)/i);
  if (!match) return 0;
  const num = parseInt(match[1], 10);
  const unit = match[2].toLowerCase();
  if (unit === 'd') return num * 24 * 3600 * 1000;

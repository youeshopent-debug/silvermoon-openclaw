...
// ─── 人性化后处理（延迟加载，仅银月回复时启用） ────────────
let _mindEngine = null;
function getMindEngine() {
  if (!_mindEngine) {
    try { _mindEngine = require('./mind-engine'); } catch {}
  }
  return _mindEngine;
}
...
function sanitize(text, opts = {}) {
  ...
  // ── 第 5.5 层：人性化后处理（仅银月回复，去除机器人口吻） ──
  if (String(opts.agent || '').trim() === '银月') {
    const engine = getMindEngine();
    if (engine && typeof engine.processReply === 'function') {
      try {
        out = engine.processReply(String(opts.userText || ''), out, String(opts.channelId || ''));
      } catch (e) {
        // 人性化失败不影响主流程
      }
    }

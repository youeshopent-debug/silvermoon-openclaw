...
const TOOLS = {
  ...
  screenshot: {
    ...
    async handler(args) {
      ...
      let result = { ok: false, path: null, engine: null, reason: '未知错误' };

      if (args.url) {
        const url = args.url.startsWith('http') ? args.url : `https://${args.url}`;
        // Plan A: chrome-cdp-bridge
        try {
          const chromeCDP = require('./chrome-cdp-bridge');
          await chromeCDP.launchOrConnect();
          await chromeCDP.navigate(url);
          const shot = await chromeCDP.screenshot(`web_${Date.now()}`);
          fs.writeFileSync(outPath, shot.buffer);
          result = { ok: true, path: outPath, engine: 'chrome-cdp-bridge', reason: null };
        } catch (e) {
          log('WARN', '截图 Plan A(chrome-cdp-bridge) 失败', { error: e?.message?.slice(0, 200) });
        }
        // Plan B: Puppeteer 直连
        if (!result.ok) {
          try {
            ...
            await page.screenshot({ path: outPath, fullPage: args.fullPage === true });
            await browser.close();
            if (fs.existsSync(outPath) && fs.statSync(outPath).size > 0) {
              result = { ok: true, path: outPath, engine: 'puppeteer', reason: null };
            }
          } catch (e) {
            log('WARN', '截图 Plan B(Puppeteer) 失败', { error: e?.message?.slice(0, 200) });
          }
        }
        // Plan C: API截图降级
        if (!result.ok) {
          try {
            const apiUrl = `https://api.screenshotone.com/take?url=${encodeURIComponent(url)}&access_key=free&viewport_width=1280&viewport_height=800&format=png`;
            const resp = await fetch(apiUrl, { signal: AbortSignal.timeout(15000) });
            if (resp.ok) {
              const buf = Buffer.from(await resp.arrayBuffer());
              fs.writeFileSync(outPath, buf);
              if (fs.existsSync(outPath) && fs.statSync(outPath).size > 0) {
                result = { ok: true, path: outPath, engine: 'screenshotone-api', reason: null };
              }
            }
          } catch (e) {
            log('WARN', '截图 Plan C(API) 失败', { error: e?.message?.slice(0, 100) });
          }
        }
        if (!result.ok) result.reason = '三路引擎均无法截图（chrome-cdp-bridge → Puppeteer → API截图服务）';
      } else {
        // 桌面截图
        // Plan A: Graphics.CopyFromScreen（不依赖剪贴板）
        try {
          const escPath = outPath.replace(/'/g, "''");
          execSync(`powershell -Command "Add-Type -AssemblyName System.Drawing; Add-Type -AssemblyName System.Windows.Forms; $bounds = [System.Windows.Forms.Screen]::PrimaryScreen.Bounds; $bitmap = New-Object System.Drawing.Bitmap $bounds.Width, $bounds.Height; $graphics = [System.Drawing.Graphics]::FromImage($bitmap); $graphics.CopyFromScreen($bounds.X, $bounds.Y, 0, 0, $bounds.Size); $bitmap.Save('${escPath}', [System.Drawing.Imaging.ImageFormat]::Png); $graphics.Dispose(); $bitmap.Dispose()"`, {
            timeout: 15000, windowsHide: true
          });
          if (fs.existsSync(outPath) && fs.statSync(outPath).size > 0) {
            result = { ok: true, path: outPath, engine: 'desktop-copyfromscreen', reason: null };
          }
        } catch (e) {
          log('WARN', '桌面截图 Plan A(CopyFromScreen) 失败', { error: e?.message?.slice(0, 200) });
        }
        // Plan B: PrtSc+Clipboard
        if (!result.ok) {
          try {
            execSync(`powershell -Command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.SendKeys]::SendWait('{PRTSC}'); Start-Sleep -Milliseconds 800; $img = [System.Windows.Forms.Clipboard]::GetImage(); if($img){ $img.Save('${outPath.replace(/'/g, "''")}','png') }"`, {
              timeout: 10000, windowsHide: true
            });
            if (fs.existsSync(outPath) && fs.statSync(outPath).size > 0) {
              result = { ok: true, path: outPath, engine: 'desktop-printscreen', reason: null };
            }
          } catch (e) {
            log('WARN', '桌面截图 Plan B(PrtSc) 失败', { error: e?.message?.slice(0, 200) });
          }
        }
        if (!result.ok) result.reason = '桌面截图引擎均失败（CopyFromScreen → PrtSc均不可用）。可尝试带网址：截图 https://example.com';
      }
      return result;
    }
  },
  ...
  turix_cua: {
    ...
    async handler(args) {
      ...
      try {
        ...
        // ── action: open — 通过 chrome-cdp-bridge 启动/连接 Chrome ──
        if (action === 'open' || url || (task && task.match(/打开|开启|open|launch|start|chrome|browser/i))) {
          const targetUrl = url || (task ? task.replace(/打开|开启|open|launch|start|chrome|browser|浏览器|访问|搜索|查看|并|然后|接着|再|and\s*/gi, '').trim() : '');
          try {
            const chromeCDP = require('./chrome-cdp-bridge');
            const result = await chromeCDP.launchOrConnect();
            if (targetUrl && !targetUrl.match(/^--/)) {
              const fullUrl = targetUrl.match(/^https?:\/\//) ? targetUrl : `https://${targetUrl}`;
              const navResult = await chromeCDP.navigate(fullUrl);
              steps.push(`🌐 Chrome ${result.mode} → 导航至 ${fullUrl}（标题: ${navResult.title}）`);
            } else {
              const info = await chromeCDP.getPageInfo();
              steps.push(`🌐 Chrome ${result.mode} → 已连接（当前: ${info.url}）`);
            }
          } catch (e) {
            steps.push(`❌ Chrome 启动失败: ${e.message.slice(0, 120)}`);
          }
        }
...
`;
  ...
  list_agents: {
    description: '列出 sects/ 下所有子代理及其职责（一行一个，技能限短名称）',
    args: {},
    async handler() {
      const sectsDir = path.join(PROJECT_ROOT, 'sects');
      ...
      const lines = ['📋 银月钱庄子代理清单（共' + items.length + '人）:'];
      for (const item of items) {
        const taskPath = path.join(sectsDir, item, 'TASK.json');
        let role = '未知', skills = [];
        if (fs.existsSync(taskPath)) {
        ...
        const shortSkills = skills.slice(0, 3).map(s => {
          const ci = s.indexOf('：') !== -1 ? s.indexOf('：') : s.indexOf(':');
          const name = ci !== -1 ? s.slice(0, ci).trim() : s.trim();
          return name.length > 25 ? name.slice(0, 25) + '…' : name;
        });
        lines.push(`   • ${item}（${role}）：${shortSkills.join('、') || '无技能'}`);
      }
      return lines.join('\n');
    }
  },
  ...
  scrapling_fetch: {
    description: '调用 Scrapling 智能爬虫 — 自适应解析/反爬绕过(Cloudflare Turnstile)/浏览器渲染/Stealth隐身。三级降级策略: get(最快纯HTTP)→fetch(浏览器渲染)→stealthy(隐身浏览器+指纹随机化,最强反爬)。基于Python Scrapling v0.4.8',
    args: {
      url: '目标网址',
      mode: '爬取模式: get(纯HTTP最快,适合简单页), fetch(浏览器渲染,适合JS动态页), stealthy(隐身浏览器+指纹随机化,最强反爬,适合Cloudflare保护页)',
      selector: 'CSS选择器（可选，提取指定区域）',
      ai_targeted: '是否启用AI定向提取减少token消耗（true/false，默认true，可防Prompt注入）',
      output_file: '保存路径（可选，默认自动生成到 temp/，不传则自动清理）'
    },
    async handler(args) {
      const url = args.url || '';
      if (!url) return '❌ 请提供目标网址（url 参数）';
      const mode = (args.mode || 'get').toLowerCase();
      if (!['get', 'fetch', 'stealthy'].includes(mode)) return '❌ mode 必须是 get/fetch/stealthy 之一';

      const cmdMap = { get: 'get', fetch: 'fetch', stealthy: 'stealthy-fetch' };
      const subCmd = cmdMap[mode];
      const ts = Date.now();
      const outFile = args.output_file || path.join(PROJECT_ROOT, 'temp', `scrapling_${ts}.md`);
      const outDir = path.dirname(outFile);
      if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });

      let cmd = `scrapling extract ${subCmd} "${url}" "${outFile}"`;
      if (args.selector) cmd += ` --selector "${args.selector}"`;
      if (args.ai_targeted !== 'false') cmd += ' --ai-targeted';

      const startTime = Date.now();
      try {
        const stdout = execSync(cmd, { timeout: 120000, encoding: 'utf-8', maxBuffer: 2 * 1024 * 1024 });
        const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);

        if (!fs.existsSync(outFile)) {
          return `❌ Scrapling 执行完成但未生成输出文件\n   ⏱️ ${elapsed}s\n   输出: ${stdout.slice(0, 500)}`;
        }

        const content = fs.readFileSync(outFile, 'utf-8').slice(0, 8000);
        const size = fs.statSync(outFile).size;

        // 自动清理临时文件（除非用户指定了 output_file）
        if (!args.output_file) {
          try { fs.unlinkSync(outFile); } catch {}
        }

        return `✅ Scrapling ${mode} 爬取成功\n   📍 ${url}\n   ⏱️ ${elapsed}s\n   📐 大小: ${(size/1024).toFixed(1)}KB\n\n${content.slice(0, 6000)}`;
      } catch (e) {
        const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
        let partial = '';
        if (fs.existsSync(outFile)) {
          partial = fs.readFileSync(outFile, 'utf-8').slice(0, 1000);
          if (!args.output_file) try { fs.unlinkSync(outFile); } catch {}
        }
        const errMsg = e.stderr ? e.stderr.slice(0, 500) : e.message.slice(0, 300);
        return `❌ Scrapling ${mode} 失败 (${elapsed}s)\n   ${errMsg}${partial ? '\n   部分内容:\n' + partial : ''}\n💡 提示: 若爬取复杂页面请试试 mode:fetch 或 mode:stealthy`;
      }
    }
  },
...
// ─── camelCase 别名，兼容银月人设提示词中的工具名 ────────────
TOOLS.discoverAgent = {
  description: '查子代理的 TASK.json（角色/技能/依赖）——camelCase 别名',
  args: { name: '子代理名称（如 墨影、药老、萧炎）' },
  async handler(args) { return TOOLS.discover_agent.handler(args); }
};
TOOLS.listAllAgents = {
  description: '列出 sects/ 下所有子代理及其职责——camelCase 别名',
  args: {},
  async handler() { return TOOLS.list_agents.handler({}); }
};
TOOLS.dispatchTask = {
  description: '向子代理派发任务——camelCase 别名',
  args: { target: '目标子代理名称', task: '任务描述', priority: '优先级: high/medium/low' },
  async handler(args) { return TOOLS.dispatch_task.handler(args); }
};
TOOLS.getDispatchHistory = {
  description: '查看派发历史和消费状态——camelCase 别名',
  args: { limit: '返回条数（默认 20）' },
  async handler(args) { return TOOLS.get_dispatch_history.handler(args); }
};
...
function parseToolBlocksLoose(text) {
  const blocks = [];
  const lines = text.split('\n');
  let i = 0;
  while (i < lines.length) {
    const toolMatch = lines[i].match(/^\s*工具:\s*(\S+)\s*$/);
    if (toolMatch) {
      const name = normalizeToolName(toolMatch[1]);
      const args = {};
      i++;
      while (i < lines.length) {
...
async function processToolCalls(text) {
  ...
  const looseBlocks = blocks.length === 0 ? parseToolBlocksLoose(text) : [];
  const allBlocks = blocks.length > 0 ? blocks : looseBlocks;
  
  if (allBlocks.length === 0) return { cleaned: text, results: [] };
  ...
  const cleaned = blocks.length > 0 ? stripToolBlocks(text) : text;
...
// 给 main.js 提供 handlers 快捷访问
const handlers = {};
for (const [name, tool] of Object.entries(TOOLS)) {
  if (tool.handler) handlers[name] = tool.handler;
}
